...
- Stepper: any standing violation sends the loop back to Inspect with the finding as target, whether the violation is new, already viewed, or persisting across evaluations, and ahead of any resolved-finding message. Previously only a never-viewed violation held the loop, so a viewed one let the hint move on to Optimize.
