...
export function InstanceSheet({ session, ctx, lifecycles, dispatch }: { session: Session; ctx: Ctx; lifecycles: LifecycleFinding[]; dispatch: React.Dispatch<Action> }) {
  ...
  return (
    <section className="sheet" id="panel-inspector">
      ...
      <h5>Connections</h5>
      <ul className="list">{pins.map(({ p, net, v }) => <li key={p.name} className="clickable" onClick={() => (net ? dispatch({ type: 'SELECT_NET', id: net.id }) : dispatch({ type: 'SELECT_PIN', pin: { instance: id, pin: p.name } }))}><span><code>{p.name}</code> <span className="muted small">{p.role.replace('_', ' ')}</span></span><span className="muted small">{net ? `${net.name}${v !== undefined ? ` · ${v.toFixed(2)} V` : ''}` : 'unconnected · tap to wire'}</span></li>)}</ul>
