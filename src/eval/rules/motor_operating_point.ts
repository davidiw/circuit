...
export const motor_operating_point: Rule = {
  ...
  analyze(ctx) {
    ...
    for (const m of motors) {
      ...
      const railNet = vmot && ctx.netOf(drvPin.instance, vmot.name); const rail = railNet && ctx.netVoltages.get(railNet.id);
      ...
      if (!seen.has(railNet!.id)) {
        seen.add(railNet!.id);
        metrics.push({ key: 'motor_rail_v', label: 'Motor rail', value: rail.nominal, unit: 'V', provenance: 'fixture_assumption', group: 'performance' }, { key: 'motor_rated_v', label: 'Motor rated voltage', value: nominal.value, unit: 'V', provenance: nominal.provenance as 'vetted_source', group: 'performance' });
        if (rpm) metrics.push({ key: 'motor_no_load_rpm_est', label: 'Estimated no-load speed', value: Math.round(rpm.value * ratio), unit: 'rpm', provenance: 'fixture_assumption', group: 'performance', note: `${rpm.value} rpm rated at ${nominal.value} V, scaled linearly with rail voltage` });
      }
      if (ratio < 0.95 && !findings.length) findings.push(finding({
        ...
        evidence: [{ label: `${railNet!.name} voltage`, value: fmtV(rail.nominal), provenance: 'fixture_assumption' }, { label: `${m.id} nominal_v`, value: fmtV(nominal.value), provenance: nominal.provenance }, ...(rpm ? [{ label: 'no_load_rpm at rated voltage', value: String(rpm.value), provenance: rpm.provenance }] : [])],
