...
export const driver_load_current: Rule = {
  ...
  analyze(ctx) {
    ...
    for (const m of motors) {
      ...
      if (!stall || !limit) { findings.push(finding({ ruleId: 'driver_load_current', basis: 'component_spec', severity: 'unknown', category: 'current', title: `${m.label}: ${!stall ? 'stall current' : 'driver channel limit'} unknown`, affected: [{ instanceId: m.id }], evidence: [], consequence: 'Driver sizing cannot be confirmed.', remediation: ['Add the missing fact to the registry'], missing: [!stall ? 'stall_current_a' : 'continuous_current_per_channel_a'] })); continue; }
      checked++;
