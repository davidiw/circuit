...
export const rail_budget: Rule = {
  ...
  analyze(ctx) {
    ...
    for (const reg of regs) {
      ...
      metrics.push({ key: `worst_case_draw_a:${reg.id}`, label: `Worst-case draw on ${reg.label}`, value: Math.round(total * 100) / 100, unit: 'A', provenance: parts.some((x) => x.provenance !== 'vetted_source') ? 'fixture_assumption' : 'vetted_source', group: 'electrical' }, { key: `rail_limit_a:${reg.id}`, label: `${reg.label} continuous limit`, value: limit.value, unit: 'A', provenance: limit.provenance, group: 'electrical' });
